...
u32 CFLVParser::GetPreviousTagSize(u8* &pbyBufferStart)
{
	u32 dwPreviousTagSize = 0;
	u8* byBufferTemp = NULL;
	if (SUCCESS == m_pobjCFileIO->FetchBuffer(byBufferTemp,sizeof(u32)))
	{
		dwPreviousTagSize = m_bitConverter.ToUINT32(byBufferTemp,0);
		pbyBufferStart = byBufferTemp;
		return dwPreviousTagSize;
	}
	return 0;
}
...
{
	u8 byRetValue = FAILURE;	
	u8* pbyBufferStart = NULL;
	u8* pbyBufferPointer = NULL;
	m_dwCurrentOffset = m_pobjCFileIO->GetOffset();
	...
	if(m_dwFileSize - m_dwCurrentOffset >= FLV_TAG_HEADER_SIZE)
	{
		if(!m_bIsFirstPacket)
		{
			FLVChunk.m_uiPreviousTagSize = GetPreviousTagSize(pbyBufferStart);
			if(NULL != pbyBufferStart)
			{
				FLVChunk.m_pbyBufferStart = pbyBufferStart;
			}
		}		

		if (SUCCESS == m_pobjCFileIO->FetchBuffer(pbyBufferPointer,FLV_TAG_HEADER_SIZE))
		{
			u8 unData;
			CFLVTags FLVTags;
			CFLVAudioData FLVAudio;
			...
			if(m_bIsFirstPacket)
			{
				pbyBufferStart = pbyBufferPointer;
				FLVChunk.m_pbyBufferStart = pbyBufferStart;
				m_bIsFirstPacket = false;
			}

			FLVTags.m_uiTagType = (unsigned _int8)pbyBufferPointer[0];
			FLVTags.m_uiDataSize = m_bitConverter.ToUINT24(&pbyBufferPointer[1],0);
			FLVTags.m_uiTimeStamp = m_bitConverter.ToUINT24(&pbyBufferPointer[4],0);
			FLVTags.m_uiTimeStampExtended = (unsigned _int8)pbyBufferPointer[7];
			FLVTags.m_uiStreamID = m_bitConverter.ToUINT24(&pbyBufferPointer[8],0);

			if (0 == FLVTags.m_uiStreamID)
			{
				switch(FLVTags.m_uiTagType)
				{
				case FLV_TAG_TYPE_AUDIO:
					if (SUCCESS == m_pobjCFileIO->FetchBuffer(pbyBufferPointer,FLV_AUDIO_HEADER_SIZE))
					{						
						unData = pbyBufferPointer[0];
						FLVAudio.m_sound = unData;						
						if(SUCCESS == m_pobjCFileIO->FetchBuffer(pbyBufferPointer,FLVTags.m_uiDataSize-FLV_AUDIO_HEADER_SIZE))
						{
							FLVChunk.m_enmChunkType = FLV_CHUNK_AUDIO;
							FLVChunk.m_pobjFLVTag = FLVTags;
							FLVChunk.m_pobjFLVAudio = FLVAudio;
							FLVChunk.m_pobjFLVVideo = FLVVideo;
							FLVChunk.m_pbyDataBuffer = pbyBufferPointer;
							FLVChunk.m_dwDataBufferLength = FLVTags.m_uiDataSize-FLV_AUDIO_HEADER_SIZE;
							byRetValue = SUCCESS;
						}						
					}
					break;
				case  FLV_TAG_TYPE_VIDEO:
					if (SUCCESS == m_pobjCFileIO->FetchBuffer(pbyBufferPointer,FLV_VIDEO_HEADER_SIZE))
					{						
						unData = pbyBufferPointer[0];
						FLVVideo.m_video.video = unData;
						u8 unTemp = FLVVideo.m_video.stVideo.uiCodecID;
						FLVVideo.m_video.stVideo.uiCodecID = FLVVideo.m_video.stVideo.uiFrameType;
						FLVVideo.m_video.stVideo.uiFrameType = unTemp;

						if(SUCCESS == m_pobjCFileIO->FetchBuffer(pbyBufferPointer,FLVTags.m_uiDataSize-FLV_VIDEO_HEADER_SIZE))
						{
							FLVChunk.m_enmChunkType = FLV_CHUNK_VIDEO;
							FLVChunk.m_pobjFLVTag = FLVTags;
							FLVChunk.m_pobjFLVAudio = FLVAudio;
					...
					break;
				case FLV_TAG_TYPE_META:
					if(SUCCESS == m_pobjCFileIO->FetchBuffer(pbyBufferPointer,FLVTags.m_uiDataSize))
					{						
						FLVChunk.m_enmChunkType = FLV_CHUNK_META;
						FLVChunk.m_pobjFLVTag = FLVTags;
						FLVChunk.m_pobjFLVAudio = FLVAudio;
